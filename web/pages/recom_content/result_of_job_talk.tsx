...
function ResultOfJobTalk() {
  const router = useRouter();
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(menuActions.setMenu("/recom_content/result_of_job_talk"))
  ...
  return (router.isFallback)
    ...
    : <>
      <InfoComponent projectName="컨텐츠 추천" functionName="취업톡톡 추천" desc={ Description }/>
      <SingleParamForm<InputModel, OutputModel>
        nCols={ 1 }
        fields={ fields }
        endpointApi={ "/api/recom_content/result_of_job_talk" }
        style={{ margin: "30px 0" }}
      />
      <Divider/>
...
export default ResultOfJobTalk;
