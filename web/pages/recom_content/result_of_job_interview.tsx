...
function ResultOfJobInterview() {
  const router = useRouter();
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(menuActions.setMenu("/recom_content/result_of_job_interview"))
  ...
  return (router.isFallback)
    ...
    : <>
      <InfoComponent projectName="컨텐츠 추천" functionName="직무 인터뷰 추천" desc={ Description }/>
      <SingleParamForm<InputModel, OutputModel>
        nCols={ 1 }
        fields={ fields }
        endpointApi={ "/api/recom_content/result_of_job_interview" }
        style={{ margin: "30px 0" }}
      />
      <Divider/>
...
export default ResultOfJobInterview;
