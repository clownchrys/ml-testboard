...
function ResultOfPassletter() {
  const router = useRouter();
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(menuActions.setMenu("/recom_content/result_of_passletter"))
  ...
  return (router.isFallback)
    ...
    : <>
      ...
      <SingleParamForm<InputModel, OutputModel>
        nCols={ 1 }
        fields={ fields }
        endpointApi={ "/api/recom_content/result_of_passletter" }
        style={{ margin: "30px 0" }}
      />
      <Divider/>
...
export default ResultOfPassletter;
